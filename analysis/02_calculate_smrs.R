...
smrs_ni <- data %>%
  ...
  group_by(
    care_site_id, care_site_name, Registry,
...
mice_summary <- mice_long %>%
  ...
  filter(.imp != 0) %>%
  group_by(person_id, visit_occurrence_id, visit_detail_id, care_site_id,
           admission_year) %>%
  summarise(
...
### Deciding if sites contributed data each month or not month, or not by
### comparing number of admissions over time.
### This decides whether or not a site should contribute to the annual SMRs
### on the funnel plots, and in the graphs.
patients_per_month_care_site_non_summarised <-
  data %>%
  mutate(
    care_site_id_fac = factor(care_site_id),
  ...
  group_by(care_site_id_fac,
    Registry, care_site_id, admission_year, admission_month,
    .drop = FALSE
  ) %>%
...
#### Fix when moving to country only.
output <- get_unique_count(data, "admission_year", "care_site_id",
                           "Number of sites", output)
