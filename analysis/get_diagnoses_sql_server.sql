...
AS (
	...
	INNER JOIN {schema}.condition_occurrence c
		...
		AND (
			adm.visit_detail_id = c.visit_detail_id
	...
	INNER JOIN {schema}.concept ON concept.concept_id = c.condition_concept_id
	)
	,
	----- Procedure data
procedure_data
AS (
	...
	INNER JOIN {schema}.procedure_occurrence p
		...
		AND (
			adm.visit_detail_id = p.visit_detail_id
			OR adm.visit_detail_id IS NULL
			)
		--- Only want data recorded on the day of admission
		AND DATEDIFF(dd, adm.icu_admission_datetime, COALESCE(p.procedure_datetime, p.procedure_date)) = 0
	--- getting source IDs to use when getting diagnosis coefficents.
	INNER JOIN {schema}.concept ON concept.concept_id = p.procedure_concept_id
	)
SELECT *
FROM condition_data
